...
@st.composite
def otlp_attribute(draw) -> dict[str, Any]:
    """
...
@st.composite
def otlp_span(draw, parent_span_id: str | None = None, trace_id: str | None = None) -> dict:
    """
    ...
    attributes = draw(st.lists(otlp_attribute(), min_size=num_attributes, max_size=num_attributes))

    # Status code
    status_code = draw(st.sampled_from([
        "STATUS_CODE_UNSET",
        "STATUS_CODE_OK",
        "STATUS_CODE_ERROR"
    ]))
...
def rf_suite_span(draw, parent_span_id: str | None = None, trace_id: str | None = None) -> dict:
    ...
    suite_id = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(
        whitelist_categories=("Lu", "Ll", "Nd"), whitelist_characters="-_"
    )))
...
def rf_test_span(draw, parent_span_id: str | None = None, trace_id: str | None = None) -> dict:
    ...
    test_id = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(
        whitelist_categories=("Lu", "Ll", "Nd"), whitelist_characters="-_"
    )))
    ...
    # Add tags if present
    if tags:
        rf_attributes.append({
            "key": "rf.test.tags",
            "value": {"string_value": ",".join(tags)}
        })
...
def rf_keyword_span(draw, parent_span_id: str | None = None, trace_id: str | None = None) -> dict:
    ...
    keyword_type = draw(st.sampled_from([
...
def ndjson_line(draw, span_strategy=None) -> str:
    ...
    resource_attributes = draw(st.lists(
        otlp_attribute(),
        min_size=num_resource_attrs,
        max_size=num_resource_attrs
    ))
...
def rf_span_tree(draw) -> list[dict]:
    ...
    for _ in range(num_tests):
        ...
        for _ in range(num_keywords):
            keyword_span = draw(rf_keyword_span(
