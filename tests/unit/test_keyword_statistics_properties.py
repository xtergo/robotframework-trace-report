...
def rf_keyword_strategy(draw, name=None, max_depth=2, current_depth=0):
    ...
    if current_depth < max_depth:
        ...
        for _ in range(num_children):
            ...
            children.append(
                draw(rf_keyword_strategy(name=child_name, max_depth=max_depth, current_depth=current_depth + 1))
            )
    
    return RFKeyword(
        name=name,
        keyword_type=draw(st.sampled_from(["KEYWORD", "SETUP", "TEARDOWN", "FOR", "IF", "TRY", "WHILE"])),
...
def rf_suite_with_keywords_strategy(draw, max_depth=2, current_depth=0):
    ...
    if current_depth < max_depth:
        ...
        for _ in range(num_suites):
            children.append(
                draw(rf_suite_with_keywords_strategy(max_depth=max_depth, current_depth=current_depth + 1))
            )
