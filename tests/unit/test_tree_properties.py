...
def get_parent_child_relationships(roots: list[SpanNode]) -> dict[str, list[str]]:
    ...
    def traverse(node: SpanNode):
        if node.children:
            relationships[node.span.span_id] = [child.span.span_id for child in node.children]
        for child in node.children:
            traverse(child)
...
def test_property_tree_reconstruction_round_trip(otlp_spans: list[dict]):
    ...
    for parent_id, child_ids in original_relationships.items():
        assert parent_id in rebuilt_relationships, f"Parent {parent_id} not found in rebuilt tree"
    ...
    for parent_id in rebuilt_relationships:
        assert parent_id in original_relationships, f"Unexpected parent {parent_id} in rebuilt tree"

    # Verify that all deduplicated spans are present in the rebuilt tree
    flattened_spans = flatten_tree(rebuilt_roots)
    assert len(flattened_spans) == len(
        deduplicated_spans
    ...
    assert (
        rebuilt_span_ids == original_span_ids
...
@given(
    st.lists(
        st.builds(
            ...
            start_time_unix_nano=st.integers(
                min_value=1000000000000000000, max_value=2000000000000000000
            ),
            end_time_unix_nano=st.integers(
                min_value=1000000000000000000, max_value=2000000000000000000
            ),
            attributes=st.just({}),
...
def test_property_all_roots_preserved(spans: list[RawSpan]):
    ...
    assert len(roots) == len(spans), f"Expected {len(spans)} roots, got {len(roots)}"

    # Each root should have no children (since no parent-child relationships exist)
    for root in roots:
        assert len(root.children) == 0, f"Root {root.span.span_id} should have no children"
    ...
    assert (
        root_span_ids == original_span_ids
...
def test_property_single_parent_multiple_children(trace_id: str, num_children: int):
    ...
    assert (
        rebuilt_child_ids == expected_child_ids
    ), f"Child IDs mismatch: expected={expected_child_ids}, got={rebuilt_child_ids}"

    # Each child should have no children
    for child in root.children:
        assert len(child.children) == 0, f"Child {child.span.span_id} should have no children"
...
def test_property_root_span_identification(otlp_spans: list[dict]):
    ...
    for root in roots:
        assert (
            root.span.span_id not in non_root_ids
...
def test_property_orphan_spans_become_roots(
    ...
    for orphan_id in orphan_ids:
        assert orphan_id in root_ids, f"Orphan span {orphan_id} should be a root"
...
def test_property_no_cross_trace_contamination(num_traces: int):
    ...
    assert len(roots) == num_traces, f"Expected {num_traces} roots, got {len(roots)}"

    # Verify that each root has a unique trace_id
    root_trace_ids = [root.span.trace_id for root in roots]
    assert (
        len(set(root_trace_ids)) == num_traces
